...
<li><input type="button" class="active btn " value="Step 6" style="  background-color: whitesmoke; width:100px;border-radius: 15px 0 15px 0; "></li>
    ...
    <?php if (count($data) > 0) : ?>
      <table style="margin-bottom:7%">
            <thead>
                <tr>
                <th>Student Name</th>
                    <th>No.Agreement 1</th>
                    <th>No.Response 1</th>
                    <th>Inter-rater1 (%)</th>
                    <th>No.Agreement 2</th>
                    <th>No.Response 2</th>
                    <th>Inter-rater2 (%)</th>
                    <th>Average (%)</th>
                    <th>Feedback</th>
            ...
            <tbody>
                <?php foreach ($data as $item) : ?>
                  <?php $totalagree=$item['noAgreement1']+$item['noAgreement2'];?>
                  <?php $totalresp=$item['noResponse1']+$item['noResponse2'];?>

                    <tr>
                    <td><?php echo $item['Fname'], '&nbsp', $item['Lname']; ?></td>
                        <td><?php echo $item['noAgreement1']; ?></td>
                        <td><?php echo $item['noResponse1']; ?></td>
                        <td><?php echo number_format((float)($item['noAgreement1']/$item['noResponse1'])*100, 1, '.', ''); ?></td>
                        <td><?php echo $item['noAgreement2']; ?></td>
                        <td><?php echo $item['noResponse2']; ?></td>
                        <td><?php echo number_format((float)($item['noAgreement2']/$item['noResponse2'])*100, 1, '.', ''); ?></td>
                        <td><?php echo number_format((float)($totalagree/$totalresp)*100, 1, '.', ''); ?></td>
                        <td>
                                <a href="mailto:<?php echo $item['user_name']; ?>?subject=Feedback%20for%20Step%206">
